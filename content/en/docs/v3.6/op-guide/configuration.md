...
{{% alert color="info" %}}
Note: When configuring via the config file, both `experimental-snapshot-catch-up-entries` and `experimental-snapshot-catchup-entries` are accepted. However, only `--snapshot-catchup-entries` is supported as a command-line flag in v3.6.
{{% /alert %}}
